...
* **Image Prune (Admin):** The images tab must offer a bulk prune action, available only to admin users, that deletes all images not used by any container in a single daemon call. The action must require an explicit confirmation step stating the scope from the current listing (how many images and how much space are reclaimable), stay disabled when nothing is reclaimable, surface daemon errors without losing the table, report the actual reclaimed space from the daemon response on success, and refresh the inventory and summary stats afterwards. Volumes and networks remain read-only in this phase.
