...
### STORY-069: Volume Usage On Demand — Sizes, Reclaim & Count (issue #212) [DONE]
...
- **Notes:** Wire verified against moby v28.5.2: `GET /system/df?type=volume` walks every local volume's directory tree **serially and uncached** (singleflight deduplicates concurrent calls only) — a seconds-scale op on large volumes — so measurement is strictly button-triggered; sizes `-1` = per-volume walk failure; `ref_count` counts running+stopped container config references but is only available in the same df response. `POST /volumes/prune` with all=true (named volumes included - anonymous-only without it) deletes only volumes unreferenced **at prune time** and reports `PruneReport{VolumesDeleted []string, SpaceReclaimed}` — stale previews can never cause a wrong deletion.
- **Acceptance Criteria:**
  ...
  6. Daemon-down → `CodeUnavailable` (both RPCs); backend tests assert `type=volume`+`verbose=1` query and report mapping.
- **Tasks:**
  ...
  3. `internal/admin/service.go`: `GetVolumeUsage` = `DiskUsage{Volumes: true, Verbose: true}` mapped by name (`-1` and nil `UsageData` passthrough, aggregates computed server-side); `PruneVolumes` = `VolumePrune(ctx, VolumePruneOptions{All: true})` mapped to count/names/bytes. Tests: mapping incl. `-1`/nil, aggregates, `type=volume` query assertion, prune wire + report, daemon-down.
