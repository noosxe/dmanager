...
- `GetVolumeUsage` → `GetVolumeUsageResponse{repeated VolumeUsage volumes, total_size_bytes, reclaimable_bytes, unused_count}` with `VolumeUsage{name, size_bytes, ref_count}` — one `DiskUsage{Volumes: true, Verbose: true}` daemon call (Verbose is required on API >= 1.52 — the non-verbose decode populates the aggregates but drops Items), mapped by volume name; `size_bytes: -1` passes through (the client renders an em dash) and a nil `UsageData` maps as unknown. The aggregates are computed server-side from the same response (`reclaimable_bytes` = sum of sizes where `ref_count == 0`; `unused_count` = that set's cardinality) so clients never re-derive them. The walk failure mode is per-volume (`-1`), not per-request: one unreadable volume does not fail the RPC.
...
**Testing.** Backend: mapping (size, `-1` passthrough, nil `UsageData`, ref counts), aggregate computation, `type=volume` + `verbose=1` query assertions on the fake daemon, prune wire call + report mapping (names preserved), daemon-down. Frontend: tab open fires no usage RPC (regression guard); Calculate sizes populates the column (`—` before, bytes after, `—` on `-1`); viewer can measure but not reclaim (`title="Admin role required"`); dialog copy switches between measured/not-measured arms; confirm flow (`{}` on the wire), toast contents, post-prune re-measure only-when-previously-measured; the count card renders from the list without any usage call.

### 9.12. Networks Tab — In-Use Visibility, Deletion & Reclaim (issue #215)

The Networks tab has been read-only since §9.5. Three gaps close in one story: in-use state is invisible, unused networks have no delete path, and `docker network prune` has no UI equivalent. The economics are the **inverse of volumes (§9.11)**: the list endpoint is blind but inspection is nearly free, so everything ships in the read path with **no opt-in gating at all**.

**Wire facts (moby v28.5.2, client v0.5.1 — verified in source).** The daemon's `GET /networks` populates the `Containers` map only for API < 1.28 clients (`Detailed: versions.LessThan(ctx, "1.28")` in the network router) — modern list responses carry empty maps, so in-use info requires one `NetworkInspect` per network. That inspect reads libnetwork's in-memory endpoint store (`buildContainerAttachments`) with **no filesystem recursion**, costing microseconds per network — dozens of inspects stay under the list round-trip's own latency. A **stopped container still counts**: its endpoint persists until `docker rm`, identical semantics to the volume `ref_count`. Daemon protections: delete of an in-use network fails with `ActiveEndpointsError` (403, "network has active endpoints"); delete of a pre-defined network (`bridge`, `host`, `none` on Linux) fails with 403 "is a pre-defined network and cannot be removed"; local-path prune skips config-only, non-pruneable and endpoint-carrying networks while the cluster path additionally skips `ingress` — all re-evaluated **at prune time**. The prune report is `PruneReport{NetworksDeleted []string}` — **no space-reclaimed figure exists** (the daemon's prune event records `reclaimed: "0"`). Client calls: `NetworkInspect(ctx, id, NetworkInspectOptions{})` → `Inspect.Containers`; `NetworkRemove(ctx, id, NetworkRemoveOptions{})`; `NetworkPrune(ctx, NetworkPruneOptions{})` → report.

**Backend.** `ListNetworks` keeps its cheap list call and enriches the result: one `NetworkInspect` per listed network sets `containers_count = len(Inspect.Containers)`; a per-network inspect failure degrades that row to `-1` (unknown, the image-usage convention) without failing the whole list. `predefined` is derived server-side with the same rule as the daemon (`bridge`/`host`/`none`) so every client sees the same delete-eligibility without re-encoding daemon policy. `DeleteNetwork{id}` (admin role) proxies `NetworkRemove`; error mapping follows the image precedent — in-use or pre-defined → `CodeFailedPrecondition` with the daemon's message, not-found → `CodeNotFound`, daemon-down → `CodeUnavailable`. `PruneNetworks` (admin role) issues `NetworkPrune` with no filters and maps the report to `networks_deleted` + `names` — deliberately no byte field, because the daemon reports none.

**Frontend — In Use column.** Slotted after **Internal**, mirroring the images table's badge styling: `containers_count > 0` → Yes (running badge), `== 0` → No (stopped badge), `-1` → `—` (unknown). Sortable — unlike volume sizes, the count is authoritative data, not a walk snapshot.

**Frontend — Actions column.** Mirrors §9.7 exactly: rows with `containers_count == 0` **and** `predefined == false` render a `Trash2` icon button for admins (spinner while that row's deletion is in flight); in-use, pre-defined, unknown-usage and viewer rows render an em dash. Clicking arms a **per-row `ConfirmDialog` inside `NetworkTable`** (the ImageTable pattern): title "Delete network?", message `Deletes network {name} ({driver}). This cannot be undone.`, confirm verb **Delete**, Cancel focus, busy lockout. The dialog is honest about blast radius: a network deletion is config, not data — containers attached at delete time would be refused by the daemon anyway, and the button only arms on rows the daemon would accept.

**Frontend — Prune row.** A single danger button **Prune Unused** in an actions row above the table (the §9.8 images pattern), admin-gated with `title="Admin role required"` for viewers, and **images-style gated on the derived scope**: disabled with `title="No unused networks to prune"` once no row qualifies (`containers_count == 0`, not pre-defined) — *unless* some row has unknown usage (`containers_count == -1`): an inspect failure hides attachments from the client while the daemon may still prune that network, so the button stays armed whenever uncertainty exists. When it does arm with a zero client-side scope, the dialog discloses `Deletes 0 unused networks` rather than refusing. The `ConfirmDialog` arm (fourth scope in `pendingPrune`): title "Prune unused networks?", message `Deletes {n} unused networks ({names or 'none found'}). In-use, pre-defined and swarm-ingress networks are never touched. This cannot be undone.` with `n` and names derived client-side from the loaded rows (the list is in memory; no server-side aggregates needed — the daemon re-evaluates protection at prune time regardless). Confirm verb **Prune**, mirroring the images bulk action.

**Toasts.** Per-row delete: `Network {name} deleted.` on success, `Failed to delete network: <daemon message>` on failure (the existing toast channel). Prune: `Deleted {n} unused networks (names, ≤ 3 listed).` — no bytes, because the daemon reports none; honesty over symmetry with the volume/image toasts. Both paths re-fetch `ListNetworks` afterwards; the table, In Use column and dialog counts all recompute from the fresh list.

**Testing.** Backend: enrichment mapping (`containers_count` from fake attachments, `-1` on inspect failure, `predefined` for bridge/host/none vs user-defined), delete wire call + error mapping (in-use, pre-defined, not-found, daemon-down), prune wire call + report mapping (names preserved). Frontend: In Use badge states incl. the `-1` dash; actions gating matrix (unused+admin → button; in-use / pre-defined / unknown / viewer → dash); per-row delete flow (dialog copy, Cancel focus, confirm → RPC, toast, refresh); prune flow (dialog copy with derived count/names, confirm, names toast, refresh); regression — opening the Networks tab still issues exactly one `ListNetworks` and no per-network inspects beyond the enrichment.
