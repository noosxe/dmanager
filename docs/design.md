...
**Frontend — Prune row.** A single danger button **Prune Unused** in an actions row above the table (the §9.8 images pattern), admin-gated with `title="Admin role required"` for viewers, **disabled only while a prune or delete is in flight** — unused networks may legitimately number zero, and the dialog says so rather than the button hiding the affordance. The `ConfirmDialog` arm (fourth scope in `pendingPrune`): title "Prune unused networks?", message `Deletes {n} unused networks ({names or 'none found'}). In-use, pre-defined and swarm-ingress networks are never touched. This cannot be undone.` with `n` and names derived client-side from the loaded rows (the list is in memory; no server-side aggregates needed — the daemon re-evaluates protection at prune time regardless). Confirm verb **Prune**, mirroring the images bulk action.
